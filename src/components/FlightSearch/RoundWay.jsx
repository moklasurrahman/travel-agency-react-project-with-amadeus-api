...
import RoundWayDatepicker from './formElements/RoundWayDatepicker';
...
const RoundWay = () => {
  ...
  const [fromAirportInputvalue, setFromAirportInputvalue] = useState(sessionStorage.getItem('fromAirportInputvalue') || 'CCO');
  const [toAirportInputvalue, setToAirportInputvalue] = useState(sessionStorage.getItem('toAirportInputvalue') || 'DXB');
  ...
  const getFlightsData = async ({ originLocationCode, destinationLocationCode, departureDate, departureReturnDate, adults, children, infants, travelClass }) => {
    setLoading(true);
    try {
      ...
      const flightResponse = await axios.get(`https://test.api.amadeus.com/v2/shopping/flight-offers?originLocationCode=${originLocationCode}&destinationLocationCode=${destinationLocationCode}&departureDate=${departureDate}&returnDate=${departureReturnDate}&adults=${adults}&children=${children}&infants=${infants}&travelClass=${travelClass}&nonStop=false`, {
      // const flightResponse = await axios.get(`https://test.api.amadeus.com/v2/shopping/flight-offers?originLocationCode=SYD&destinationLocationCode=BKK&departureDate=2024-05-02&returnDate=2024-05-04&adults=1&children=0&infants=0&travelClass=ECONOMY&nonStop=false&max=250`, {
      headers: {
      ...
      navigate(`/flights?from=${originLocationCode}&to=${destinationLocationCode}&date=${departureDate}&returnDate=${departureReturnDate}&adults=${adults}&children=${children}&infants=${infants}&travelClass=${travelClass}`, { state: { flights: flightResponse.data.data } }, { state: { error } });
    
    } catch (error) {
      // setError(error.response.data.message || 'No flights found for this itinerary');
      navigate(`/flights?from=${originLocationCode}&to=${destinationLocationCode}&date=${departureDate}&returnDate=${departureReturnDate}&adults=${adults}&children=${children}&infants=${infants}&travelClass=${travelClass}`, { state: { error: error.response.data.message || 'No flights found for this itinerary' } });
  ...
  const getFlights = async (event) => {
    ...
    const body = {
      ...
      departureReturnDate: formattedEndDate,
      adults: adultsValue,
      children: childrenValue,
      infants: infantValue,
      travelClass: selectedClassValue,
    };
    console.log("fgfg",body)
...
{/* ------------------------------DateRangePicker------------------------------------------------------- */}
          <div className="flex items-center">
            <RoundWayDatepicker setFormattedStartDate={setFormattedStartDate}
